...
Prose follows [writing-rules.md](writing-rules.md). Two campaign checkers back it: `tools/scripts/validate-prose.js` and `tools/scripts/validate-links.js` – run them on changed docs before a PR; they are not wired into CI. `validate-prose` takes file paths to check a subset. Renaming a heading removes an anchor – `node tools/scripts/validate-links.js --baseline main` lists what a branch removed.
