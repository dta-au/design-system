...
description: 'The focus indicator shows which element has focus, so every user can track their position and people with low vision can work.'
foundation-type: Accessibility
---

## Focus management in this system

The design system manages focus in every component and pattern. It provides a clear, consistent focus indicator, with no per-component customisation.
...
- Never remove a focus indicator unless an equivalent visual indicator replaces it.
- When the page removes a focused object – for example, a section alert closes – focus moves to the next appropriate element.
...
- provides a visually distinct focus state, consistent across every component
...
- use `outline` rather than `box-shadow` for the focus ring – `overflow: hidden` does not clip it, and Windows High Contrast Mode honours it
...
- use `:focus` instead of `:focus-visible` – this shows the ring on mouse clicks, which surprises sighted pointer users
- suppress the focus ring with `outline: none` or `outline: 0` without providing an equivalent replacement
- use `box-shadow` as the sole focus indicator – a parent `overflow: hidden` clips it, and Windows High Contrast Mode drops it
- change the focus colour per component – a single consistent ring aids recognition
- confuse the focus ring with `--glow` – glow signals pointer hover, and the ring signals keyboard navigation

## WCAG references

- [WCAG SC 1.4.11 Non-text Contrast](https://www.w3.org/WAI/WCAG21/Understanding/non-text-contrast.html)
- [WCAG SC 2.4.7 Focus Visible](https://www.w3.org/WAI/WCAG21/Understanding/focus-visible.html)
