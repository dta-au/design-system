...
- standards or criteria published as a set – readers often need one criterion without reading the rest. Use a card grid so they navigate straight to it
...
Forward and back controls on content that users can read in any order create false expectations. Users who see 'Next' expect to be missing something if they skip ahead. Apply chapter navigation only when that expectation is accurate.
...
A chapter page can carry both. Put chapter navigation at the very bottom for sequential traversal, and a cross-surface CTA above it. They sit in that order – sequential controls last, because sequential reading is the dominant case on a chapter page.
...
- [Section navigation](/patterns/section-navigation/) – use when readers can take the pages in any order.
