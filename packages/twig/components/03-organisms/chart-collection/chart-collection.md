...
- the panels share no story – separate figures with their own context beat a false grouping
...
The collection renders a named section with a heading, a description and a responsive grid of chart panels. Each panel is the standard Chart component in embedded mode. The collection owns the keyline border and surface; each panel keeps its own canvas, legend and data table. Panels adapt to their own grid cell width, not the viewport.
...
- The "View all as tables" control reports its state through aria-expanded. It follows along when a reader uses a panel summary instead (WCAG 4.1.2 Name, Role, Value).
